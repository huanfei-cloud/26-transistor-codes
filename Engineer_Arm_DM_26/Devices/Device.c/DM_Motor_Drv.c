...
void dm_motor_fbdata(motor_t *motor, uint8_t *rx_data)
{
	...
	if (motor->zero_flag == 0 && motor->para.pos != 0)
	{
		motor->zero = motor->para.pos;
		motor->zero_flag = 1;
	}
}
...
/**
************************************************************************
* @brief:      	enable_motor_mode: 选择模式，启用相应的id
* @param[in]:   hcan:     指向CAN_HandleTypeDef结构的指针
...
{
	uint8_t data[8];
	uint16_t id = motor_id + mode_id; // #define MIT_MODE 0x000,#define POS_MODE 0x100,#define SPD_MODE 0x200,#define PSI_MODE 0x300
	tmp_id = id;
	data[0] = 0xFF;
	data[1] = 0xFF;
	data[2] = 0xFF;
...
void read_motor_data(uint16_t id, uint8_t rid)
{
	...
	fdcanx_send_data(&hfdcan1, 0x7FF, data, 4); // 回调中读取寄存器
}
