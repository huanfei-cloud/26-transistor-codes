...
void fdcan_rx_callback(FDCAN_HandleTypeDef *hfdcan)
{
    uint16_t rec_id;
    uint8_t rx_data[8] = {0};
    ...
    case 0x01:
        ...
        // receive_motor_data(&motor[Motor1], rx_data); // fbdata获取基本参数存入para便于进行反馈控制,receive根据给定rid值获取相应参数存入tmp
        break;
    case 0x02:
        dm_motor_fbdata(&motor[Motor2], rx_data);
        // receive_motor_data(&motor[Motor2], rx_data); // fbdata获取基本参数存入para便于进行反馈控制,receive根据给定rid值获取相应参数存入tmp
        break;
    case 0x05:
        dm_motor_fbdata(&motor[Motor5], rx_data);
        //receive_motor_data(&motor[Motor3], rx_data); // fbdata获取基本参数存入para便于进行反馈控制,receive根据给定rid值获取相应参数存入tmp
        break;
    case 0x208:
    {
        get_motor_data(&joint_motor4, rx_data);
        motor_encoder_state_change(&joint_motor4);
        break;
    }

    case 0x202:
    {
        get_motor_data(&joint_motor5, rx_data);
        motor_encoder_state_change(&joint_motor5);
        break;
    }

    case 0x203:
    {
        get_motor_data(&joint_motor6, rx_data);
        motor_encoder_state_change(&joint_motor6);
        break;
    }
...
// 此函数是CAN接收FIFO0消息挂起回调函数，用于处理CAN1和CAN2的接收消息。
{
    ...
    {
        fdcan_rx_callback(&hfdcan1);
    }
    else if (hfdcan->Instance == FDCAN2)
    {
        // remote_callback(&hfdcan2);
        fdcan_rx_callback(&hfdcan2);
    }
}
