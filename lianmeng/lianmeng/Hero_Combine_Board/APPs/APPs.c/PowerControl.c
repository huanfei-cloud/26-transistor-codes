...
void PowerControl_Handle()
{
    ...
    if (Pin_all <= PowerControl_Data.Power_Available) return;
    else
    {
...
void PowerControl_Cal_Torque()
{
    ...
    for (i = 0; i <= 3; i++) arm_sqrt_f32(exp2[i], &exp1[i]);
    arm_negate_f32(w_MC, exp2, 4);            //-w/9.55

    for (i = 0; i <= 3; i++)
    {
        if (I_Array[i] <= 0) arm_sub_f32(&exp2[i], &exp1[i], &exp3[i], 1);
        else arm_add_f32(&exp2[i], &exp1[i], &exp3[i], 1);
    }

    arm_mult_f32(exp3, Div_2Kt, exp1, 4);
    arm_mult_f32(exp1, IC, exp2, 4);

    for (i = 0; i <= 3; i++) M3508_Array[i].outCurrent = (int16_t)exp2[i];
}
...
void PowerControl_Update()
{
    for (int i = 0; i <= 3; i++)
    {
        I_Array[i] = (float)M3508_Array[i].outCurrent;
        Speed_Array[i] = (float)M3508_Array[i].realSpeed;
    }
}
...
void PowerControl_MsgSend()
{
    ...
    Fdcan_Fun.FDCAN_SendData(FDCAN_SendHandle, &hfdcan1, FDCAN_STANDARD_ID, PowerBuffer_ID, data);
    ...
    Fdcan_Fun.FDCAN_SendData(FDCAN_SendHandle, &hfdcan1, FDCAN_STANDARD_ID, PowerLimit_ID, data);
}
...
void PowerControl_MsgRec(Fdcan_Export_Data_t RxMessage)
{
	...
	data = (int16_t)(RxMessage.FDCANx_Export_RxMessage[0] << 8 | RxMessage.FDCANx_Export_RxMessage[1]);
    PowerControl_Data.Cap_U = int16_to_float(data, 32000, -32000, 30, 0);
	data = (int16_t)(RxMessage.FDCANx_Export_RxMessage[2] << 8 | RxMessage.FDCANx_Export_RxMessage[3]);
    PowerControl_Data.Cap_I = int16_to_float(data, 32000, -32000, 20, -20);
	data = (uint16_t)(RxMessage.FDCANx_Export_RxMessage[4] << 8 | RxMessage.FDCANx_Export_RxMessage[5]);
    PowerControl_Data.Cap_Status_t.Cap_Status = data;
}

/**
  * @brief  根据裁判系统更新超级电容控制板的数据
  * @param
  * @retval void
  */
void PowerContol_MsgRenew(void)
{
	PowerControl_Data.PoweBuffer = ControlMes.chassis_power_buffer;
	PowerControl_Data.Power_Limit = ControlMes.chassis_power_limit;
}
