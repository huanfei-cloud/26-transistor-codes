...
static uint8_t protocol_frame_parse(uint8_t *data, uint16_t *data_len)
{
    ...
    uint16_t need_to_parse_len = *data_len;
...
int8_t receiving_process(void)
{
  ...
  while(1)
  {
    cmd_type = protocol_frame_parse(Usart_Data.Usart1_Data, &Usart_Data.Data_Size);
    switch (cmd_type)
    {
      case CMD_NONE:
      {
      ...
      case SET_P_I_D_CMD:
      {
				uint32_t ch 	 =  Usart_Data.Usart1_Data[CHX_INDEX_VAL];
        uint32_t temp0 = COMPOUND_32BIT(&Usart_Data.Usart1_Data[13]);
        uint32_t temp1 = COMPOUND_32BIT(&Usart_Data.Usart1_Data[17]);
        uint32_t temp2 = COMPOUND_32BIT(&Usart_Data.Usart1_Data[21]);
        ...
        Incremental_PIDInit(&M2006_Array_Pid[ch],p_temp, i_temp, d_temp,10000, 3000);    // 设置 P I D
      }
      break;

      case SET_TARGET_CMD:
      {
        int actual_temp = COMPOUND_32BIT(&Usart_Data.Usart1_Data[13]);    // 得到数据
        uint32_t ch 	 =  Usart_Data.Usart1_Data[CHX_INDEX_VAL];
				M2006_Array_Pid[ch].Target = actual_temp;
