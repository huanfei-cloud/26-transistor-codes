...
void M3508_Friction_getInfo(Fdcan_Export_Data_t RxMessage);
...
{
    uint32_t StdId;
    StdId = (int32_t)(RxMessage.FDCAN_RxHeader.Identifier - M3508_READID_CHASSIS_START);
    //解包数据，数据格式详见C620电调说明书P33
    M3508_Array[StdId].realAngle = (uint16_t)(RxMessage.FDCANx_Export_RxMessage[0] << 8 | RxMessage.FDCANx_Export_RxMessage[1]);
    M3508_Array[StdId].realSpeed = (int16_t)(RxMessage.FDCANx_Export_RxMessage[2] << 8 | RxMessage.FDCANx_Export_RxMessage[3]);
    M3508_Array[StdId].realCurrent = (int16_t)(RxMessage.FDCANx_Export_RxMessage[4] << 8 | RxMessage.FDCANx_Export_RxMessage[5]);
	...
	//上电更新targetAngle
	static uint8_t Motor_Flag[8]={0};
	if(!Motor_Flag[StdId])
	{
		M3508_Array[StdId].targetAngle=M3508_Array[StdId].totalAngle;
		Motor_Flag[StdId] = 1;
	}
...
void Check_WheelM3508(void)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        if (M3508_Array[i].InfoUpdateFrame < 1)
        {
            M3508_Array[i].OffLineFlag = 1;
        }
        else
        {
            M3508_Array[i].OffLineFlag = 0;
        }
        M3508_Array[i].InfoUpdateFrame = 0;
    }
	#elif(USING_BOARD == BOARD1)
	//摩擦轮和拨弹轮M3508检测
    for (int i = 4; i < 7; i++)
    {
        if (M3508_Array[i].InfoUpdateFrame < 1)
