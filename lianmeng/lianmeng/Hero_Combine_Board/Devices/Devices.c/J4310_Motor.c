...
/**
 ...
 void J4310_setParameter(float uq1, float uq2, float uq3, float uq4, float uq5, uint8_t *data)
 {
	 ...
	 P_MAX = 3.141593f; V_MAX = 200.f; T_MAX = 7.f;
...
{	
    ...
    Fdcan_Send_Data.FDCAN_TxHeader.Identifier = 0x001;
    Fdcan_Send_Data.FDCAN_TxHeader.IdType = FDCAN_STANDARD_ID;      //ID类型
   ...
   Fdcan_Send_Data.FDCANx_Send_RxMessage[1] = 0xFF;
	 ...
	 Fdcan_Send_Data.FDCANx_Send_RxMessage[7] = 0xFC;
	
	 HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2,&Fdcan_Send_Data.FDCAN_TxHeader,Fdcan_Send_Data.FDCANx_Send_RxMessage);
...
{	    
   ...
   Fdcan_Send_Data.FDCANx_Send_RxMessage[1] = 0xFF;
	 ...
	 Fdcan_Send_Data.FDCANx_Send_RxMessage[5] = 0xFF;
	 Fdcan_Send_Data.FDCANx_Send_RxMessage[6] = 0xFF;
	 Fdcan_Send_Data.FDCANx_Send_RxMessage[7] = 0xFE;
...
{
int32_t StdId;
	  StdId = (RxMessage.FDCANx_Export_RxMessage[0])&0x0F;
    StdId = (int32_t)RxMessage.FDCAN_RxHeader.Identifier - J4310_READID_PITCH; //由0开始
		float P_MAX,V_MAX,T_MAX;

		P_MAX = 12.5f; V_MAX = 200.f; T_MAX = 7.f;   //J4310电机数据
	
		J4310_Array[StdId]->lastAngle = J4310_Array[StdId]->realAngle;
    J4310_Array[StdId]->state = RxMessage.FDCANx_Export_RxMessage[0]>>4;
		J4310_Array[StdId]->angleInit = (uint16_t)((RxMessage.FDCANx_Export_RxMessage[1] <<8) | RxMessage.FDCANx_Export_RxMessage[2]);
		J4310_Array[StdId]->speedInit = (uint16_t)((RxMessage.FDCANx_Export_RxMessage[3] <<4) | (RxMessage.FDCANx_Export_RxMessage[4]>>4));
		J4310_Array[StdId]->torqueInit = (uint16_t)((RxMessage.FDCANx_Export_RxMessage[4]&0xF <<8) | RxMessage.FDCANx_Export_RxMessage[5]);
	  ...
	  J4310_Array[StdId]->torque = uint_to_float(J4310_Array[StdId]->torqueInit,-T_MAX,T_MAX,12);
		J4310_Array[StdId]->temperatureMOS   = (float)(RxMessage.FDCANx_Export_RxMessage[6]);
		J4310_Array[StdId]->temperatureRotor = (float)(RxMessage.FDCANx_Export_RxMessage[7]); 
