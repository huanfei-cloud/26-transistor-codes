...
{
    SET_BIT(hspi1.Instance->CR2, SPI_CR2_TXDMAEN);//允许通过DMA自动发送数据
    SET_BIT(hspi1.Instance->CR2, SPI_CR2_RXDMAEN);//允许通过DMA自动接收数据
    __HAL_SPI_ENABLE(&hspi1);    //使能SPI
    //失效DMA
    __HAL_DMA_DISABLE(&hdma_spi1_rx);
    
    //确保在对DMA接收配置（如更新缓冲区或数据长度）进行任何操作之前，接收DMA通道被禁用
    while(hdma_spi1_rx.Instance->CR & DMA_SxCR_EN)
    {
        __HAL_DMA_DISABLE(&hdma_spi1_rx);
    }

    //确保在配置DMA接收通道之前，清除之前的传输完成中断标志
    __HAL_DMA_CLEAR_FLAG(&hdma_spi1_rx, DMA_LISR_TCIF2);
//配置DMA接收通道，使其能从SPI数据寄存器读取数据
