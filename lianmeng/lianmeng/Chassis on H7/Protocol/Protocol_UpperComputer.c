...
float Auto_Aim_Yaw;
float Auto_Aim_Pitch = Cloud_Pitch_level;
struct Struct_PID_Manage_Object Auto_Aim_PID;
...
void UpperCom_Send_To_Up(uint8_t COM)
{
	...
	float bullet_angle = (ControlMes.pitch_encoder - Cloud_Pitch_level) / 1303.8f;
	int16_t gimbal_yaw = yaw_motor.encoder;
...
{	
	...
	switch(Rec[2])
	{
		case 0xFF:
		case 0x00:
			Auto_Aim_Pitch = ControlMes.pitch_encoder;
			Auto_Aim_Yaw = yaw_motor.encoder;
			break;
		case 0xCD:
