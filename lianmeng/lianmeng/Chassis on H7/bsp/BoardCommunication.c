...
void Board2_To_1(void)
{
	...
	bullet_speed = (int16_t)(ext_shoot_data.data.bullet_speed * 1000);
	float yaw_angle = AHRSData_Packet.Yaw  / pi * 4096;
	float Gyro_z = AHRSData_Packet.YawSpeed / pi * 180;
	uint8_t game_start=ext_game_status.data.game_progress;

	data1[0] = (int16_t)bullet_speed >> 8;
	data1[1] = (int16_t)bullet_speed;
	data1[2] = (int16_t)ControlMes.shooter_42mm_heat_limit>>8;
	data1[3] = (int16_t)ControlMes.shooter_42mm_heat_limit;
	data1[4] = (int16_t)ControlMes.shooter_42mm_heat_now>>8;
	data1[5] = (int16_t)ControlMes.shooter_42mm_heat_now;
