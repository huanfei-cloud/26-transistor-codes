...
void FDCAN_SendData(QueueHandle_t FDCANx_Handle, FDCAN_HandleTypeDef *FDCANx, uint8_t id_type, uint32_t id, uint8_t data[8])
{
  ...
  Fdcan_Send_Data.FDCAN_TxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE; // 发送节点处于错误状态
...
{
  static uint8_t Fdcan_Type = 0; // 区别Fdcan1和Fdcan2
  ...
  /*把接收的数据发生送给接收队列	*/
  if (!Fdcan_Type)
    xQueueSendToBackFromISR(FDCAN1_ReceiveHandle,
                            &Fdcan_Export_Data[Fdcan_Type],
                            &xHigherPriorityTask);
  else
    xQueueSendToBackFromISR(FDCAN2_ReceiveHandle,
                            &Fdcan_Export_Data[Fdcan_Type],
                            &xHigherPriorityTask);
